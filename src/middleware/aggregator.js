...
module.exports = function (config, reliableGet, eventHandler) {
  return function (req, res, next) {
    ...
    var getCx = function (fragment, next) {
      ...
      var onErrorHandler = function (err, oldCacheData) {
        var errorMsg,
          elapsed = Date.now() - req.timerStart,
          timing = Date.now() - start,
          msg = _.template(errorTemplate);

        options.statusCode = err.statusCode;
        ...
        if (ignoreError && (ignoreError === 'true' || _.contains(ignoreError.split(','), '' + err.statusCode))) {
          ...
          logError(err, errorMsg(opts), true);
    ...
    parxer(opts, data, function (err, content) {
      // Overall errors
      if (err && err.content) {
        if (!res.headersSent) {
